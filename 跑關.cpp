...
void groundGame(int row,int col){
	...
	int team1,team2;
	int count = 0;
	int try1[MAX][MAX]; //記錄當前回合中，已經測試過哪些組合
	...
	while(1){
		...
		if( (team1 >= teamCount) ^ (team2 >= teamCount)){
			if(!try1[team1][team2]){
				try1[team1][team2]  = try1[team2][team1] =  1; // 6 v 7  === 7 v 6, 所以count一次扣二
				count-=2;
			}
			continue;
		}
		...
		if( try1[team1][team2] || try1[team2][team1] )
			continue;
		else{
			try1[team1][team2]  = try1[team2][team1] =  1;
			count -=2;
		}
